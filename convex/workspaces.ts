...
const findByWorkspaceId = async (ctx: QueryCtx | MutationCtx, id: string) => {
  return await ctx.db
    .query("workspaces")
    .withIndex("by_workspace_id", (q: any) => q.eq("id", id))
    .first();
};
...
export const updateTunnel = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.metroPort !== undefined) patch.metroPort = args.metroPort;
