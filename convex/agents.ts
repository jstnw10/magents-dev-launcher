...
type AgentStatus = "done" | "running" | "idle" | "error";
...
const findByAgentId = async (ctx: QueryCtx | MutationCtx, id: string) => {
  return await ctx.db
    .query("agents")
...
export const update = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.metroServerUrl !== undefined) patch.metroServerUrl = args.metroServerUrl;
...
export const toggle = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const nextStatus: AgentStatus = existing.status === "done" ? "idle" : "done";
    await ctx.db.patch(existing._id, {
      status: nextStatus,
      lastUpdatedAt: Date.now(),
    });
